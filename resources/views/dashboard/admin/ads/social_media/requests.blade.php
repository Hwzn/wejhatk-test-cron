...
@toastr_css
@section('title')
    {{trans('ads_trans.Ads_SocialMedia')}}
@stop
@endsection
@section('page-header')
<!-- breadcrumb -->
<div class="page-title">
    <div class="row">
        <div class="col-sm-6">
            <h4 class="mb-0">{{trans('ads_trans.Ads_SocialMedia')}}</h4>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb pt-0 pr-0 float-left float-sm-right ">
                <li class="breadcrumb-item"><a href="#" class="default-color">{{trans('main_trans.Home')}}</a></li>
                <li class="breadcrumb-item active">{{trans('ads_trans.Ads_SocialMedia')}}</li>
            </ol>
        </div>
    </div>
</div>
...
<div class="col-xl-12 mb-30">
                    ...
                    <div class="table-responsive">
                        <table id="datatable" class="table  table-hover table-sm table-bordered p-0"
                            ...
                            <tr>
                                ...
                                <th>{{trans('ads_trans.social_media_platform')}}</th>
                                <th>{{trans('ads_trans.number_of_times_placed_ads')}}</th>
                                <th>{{trans('ads_trans.ads_type')}}</th>
                                <th>{{trans('ads_trans.ads_description')}}</th>
                                <th>{{trans('ads_trans.agency_logo')}}</th>
                                <th>{{trans('ads_trans.ads_date')}}</th>
                                <th>{{trans('ads_trans.ads_time')}}</th>
                                <th>{{trans('ads_trans.addational_information')}}</th>
                            ...
                            @foreach ($ads as $ad)
                                <tr>
                                    ...
                                    <td>{{ $ad->phone }}</td> 
                                    <td>{{ $ad->email }}</td> 
                                    <td>{{ $ad->social_media_platform }}</td> 
                                    <td>{{ $ad->number_of_times_placed_ads }}</td> 
                                    <td>{{ $ad->ads_type }}</td> 
                                    <td>{{ $ad->ads_description }}</td> 
                                    <td>
                                      @if($ad->agency_logo==1)
                                         @lang('ads_trans.yes')
                                      @else
                                    ...
                                    <td>{{ $ad->ads_date }}</td> 
                                    <td>{{ $ad->ads_time }}</td> 
                                    <td>{{ $ad->addational_information }}</td> 
                                    ...
                                    @if(!empty($ad->photo))
                                       @if(!is_null($ad->photo))
                                          <a class="btn btn-outline-info btn-sm"
                                                           href="{{route('view_adsattachment',$ad->photo)}}"
                                    ...
                                    <a class="btn btn-info btn-sm" href="#"  title="view problem > {{$ad->id}}"
                                                  data-href="{{route('showrequestad_details',$ad->id)}}"
                                                   ad-id="{{$ad->id}}" 
                                                   data-toggle="modal" 
 ...
 @include('dashboard.admin.ads.social_media.requestdetails')
...
var getHref = button.data('href'); //get button href
    ...
    $.ajax({
    ...
    }).done(function(data)
        {
            ...
            $("#requestdetail_Modola form textarea[name='descripation']").val(data.ads_description);
            $("#requestdetail_Modola form textarea[name='addational_information']").val(data.addational_information);
