...
<div class="page-title">
    <div class="row">
        <div class="col-sm-6">
            <h4 class="mb-0">{{trans('ads_trans.AdsAttributes_List')}}</h4>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb pt-0 pr-0 float-left float-sm-right ">
                <li class="breadcrumb-item"><a href="#" class="default-color">{{trans('main_trans.Home')}}</a></li>
                <li class="breadcrumb-item active">{{trans('ads_trans.AdsAttributes_List')}}</li>
            </ol>
        </div>
    </div>
</div>
...
<div class="col-xl-12 mb-30">
            <div class="card card-statistics h-100">
                <div class="card-body">
                    ...
                    <a href="#" class="button x-small" role="button" aria-disabled="true"
                       data-toggle="modal" 
                       data-target="#addadsattribute_Modola">
                       {{ trans('ads_trans.New_Attribute') }}
                     </a>
                    <br><br>
                    <div class="table-responsive">
                        <table id="datatable" class="table  table-hover table-sm table-bordered p-0"
                            ...
                            <tr>
                                <th>#</th>
                                <th>{{trans('ads_trans.Attribute_Name')}}</th>
                                <th>{{trans('ads_trans.Attribute_Type')}}</th>
                                <th>{{trans('ads_trans.Processes')}}</th>
                            </tr>
                            </thead>
                            <tbody>
                            @foreach ($adsattruibtes as $attruibte)
                                <tr>
                                    ...
                                    <a class="btn btn-info btn-sm" href="#" 
                                                   data-href="{{ route('edit_adsattrib',$attruibte->id) }}" 
        ...
        <!--includes_ -->
          @include('dashboard.admin.attributes.ads.add')
          @include('dashboard.admin.attributes.ads.edit')
          @include('dashboard.admin.attributes.ads.delete')
...
@section('js')
<script>
   $("#addadsattribute_Modola  button,#editModolad_attribute  button").click(function(e)
   {
    e.preventDefault();
     if($(this).attr('type')==='add')
       {
         url=$("#addadsattribute_Modola form").attr('action');
        
         data={
           attribute_ar:$("#addadsattribute_Modola form input[name='Name']").val(),
           attribute_en:$("#addadsattribute_Modola form input[name='Name_en']").val(),
           attribute_type:$("#addadsattribute_Modola form select[name='Attribute_Type']").val(),
  ...
  $("#editModolad_attribute").on('show.bs.modal', function(event) {
    ...
    $("#deleteModola_attribute").on('show.bs.modal', function(event) {
           ...
           delete_url="/admin/delete_adsattr/"+id;
