...
//      return response()->json($date);
     ...
     //tripagents
      ...
      $tripagents=Tripagent::select('trip_agents.id',"name as Tripagent_Name",'trip_agents.photo','starnumber','profile_photo','appearance_order','duration',
               'ads_requests.status','ads_requests.expire_at','ads_requests.apperance_status')
                  ->join('ads_requests','ads_requests.tripagent_id','=','trip_agents.id')
      // ->where('type','Tourism_Company')
      ->where('verified_at',"!=",Null)
      ->where('trip_agents.status','active')
      ->where('ads_requests.status','!=','expired')
      ->where('ads_requests.appearance_order','=',1)
      ->where('ads_requests.apperance_status','=','show')

 ->orderby('id','desc')->take(4)->get();
  return response()->json($tripagents);
